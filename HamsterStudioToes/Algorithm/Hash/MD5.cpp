...
namespace detail
{
	...
	{
		auto num = ((byts.size() + 8) / 64) + 1; // 以512位,64个字节为一组
		std::vector<uint32_t> strByte(num * 16, 0);      // 64/4=16,所以有16个整数
		for (auto i = 0u; i < byts.size(); i++)
		{
...
std::string HamsterStudioToes::MD5::ToString(Int128 i128)
{
	using namespace detail;
	return to_bytes(i128.atemp) + to_bytes(i128.btemp) + to_bytes(i128.ctemp) + to_bytes(i128.dtemp);
}

HamsterStudioToes::MD5::Int128 HamsterStudioToes::MD5::MD5(std::vector<uint8_t> const& byts)
{
	using namespace detail;
